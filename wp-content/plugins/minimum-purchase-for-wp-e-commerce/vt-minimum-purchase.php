<?php
/*
Plugin Name: VarkTech Minimum Purchase for WP E-Commerce
Plugin URI: http://varktech.com
Description: An e-commerce add-on for WP-E-Commerce, supplying minimum purchase functionality.
Version: 1.06
Author: Vark
...
= 1.02 - 2012-10-24 =
* move activation hook to is-admin
* added worldwide currency signs
* enhanced error message text and grouping  
...
*/
   ...
   $vtmin_setup_options;
//   $vtmin_error_msg;
   error_reporting(E_ERROR | E_CORE_ERROR | E_COMPILE_ERROR); //v1.06
...
class VTMIN_Controller{
    ...
    require ( VTMIN_DIRNAME . '/wpsc-integration/vtmin-parent-definitions.php');
 ...
 *************************************************** */
    ...
    require ( VTMIN_DIRNAME . '/wpsc-integration/vtmin-parent-functions.php');
    require ( VTMIN_DIRNAME . '/wpsc-integration/vtmin-parent-cart-validation.php');
  ...
  function vtmin_activation_hook() {
   ...
   if (version_compare(PHP_VERSION, VTMIN_EARLIEST_ALLOWED_PHP_VERSION) < 0) {    //'<0' = 1st value is lower 
    ...
    if(defined('WOOCOMMERCE_VERSION') && (VTMIN_PARENT_PLUGIN_NAME == 'WooCommerce')) { 
      ...
      if( (version_compare(strval($new_version), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower 
  			// delete_option('vtmin_setup_options');
  			 wp_die( __('<strong>Looks like you\'re running an older version of WooCommerce. <br>You need to be running at least ** WooCommerce 3.8 **, to use the Varktech Minimum Purchase plugin.</strong>', 'vtmin'), __('VT Minimum Purchase not compatible - WooCommerce', 'vtmin'), array('back_link' => true));
  			return;
  		}
    }   else 
