...
class VTMIN_Apply_Rules{
  ...
  public function vtmin_minimum_purchase_check() { 
            ...
            */
            ...
            switch( $vtmin_rules_set[$i]->specChoice_in_selection ) {
               ...
               case 'each': //$specChoice_value = 'each' => apply the rule to each product individually across all products found         		
                  ...
                  break;
               case 'any':  //$specChoice_value = 'any'  =>   "You may buy a minimum of $10 for each of any of 2 products from this group."       		
    ...
    */
    ...
    if ( $vtmin_setup_options['debugging_mode_on'] == 'yes' ) {
      echo '<pre>'.print_r($vtmin_rules_set, true).'</pre>' ; 
      echo '<pre>'.print_r($vtmin_cart, true).'</pre>' ;
      echo '<pre>'.print_r($vtmin_setup_options, true).'</pre>' ;
      echo '<pre>'.print_r($vtmin_info, true).'</pre>' ; 
    }
  ...
  public function vtmin_create_table_error_message () { 
   ...
   public function vtmin_table_line ($i, $k){
     ...
     $message_line .= __('<span class="product-column color-grp', 'vtmin');
     ...
     $message_line .= __('<span class="price-column">', 'vtmin');
     ...
     if ($vtmin_rules_set[$i]->amtSelected_selection == 'currency')   {
        $message_line .= __('<span class="total-column color-grp', 'vtmin');
        ...
        $message_line .= __('<span class="total-column">', 'vtmin');   
      }
   ...
   public function vtmin_table_totals_line ($i){
     ...
     $message_totals .= __('<span class="price-column">', 'vtmin');
     $message_totals .= __('&nbsp;', 'vtmin');
         ...
         case  'cart' : 
             switch( $vtmin_rules_set[$i]->specChoice_in_selection ) {
                case 'all': 
                  ...
                  break;
                case 'each':
                    $message_text .= __('for each product the cart.', 'vtmin');                             
                  break;
                case 'any':
                    $message_text .= __('for the first ', 'vtmin');
                    $message_text .= __('<span class="color-grp', 'vtmin');
   ...
   public function vtmin_create_text_error_message ($i) { 
     ...
     if  ( $vtmin_setup_options['show_error_messages_in_table_form'] == 'yes' ) {
        ...
        if ($vtmin_rules_set[$i]->amtSelected_selection == 'currency')   {
          ...
          $message .= $this->vtmin_format_money_element( $vtmin_rules_set[$i]->minimum_amt['value'] );
          switch( $vtmin_rules_set[$i]->specChoice_in_selection ) {
            case 'all': 
          ...
          $message .= $this->vtmin_format_money_element( $vtmin_rules_set[$i]->errProds_total_price );
          $message .= __(' </span></span> ', 'vtmin');
