...
if(isset($_REQUEST['action'])) {
	...
	if($_POST['action'] == 'edit') {
		...
		edit_post($_POST);
		echo "<div class='updated'><p>" . t('Template modified.') . "</p></div>";

	} elseif($_POST['action'] == 'new') {
		$_POST['post_content'] = $_POST['content'];
		wp_insert_post($_POST);
		echo "<div class='updated'><p>" . t('Template created.') . "</p></div>";

	} elseif($_GET['action'] == 'delete') {
		wp_delete_post($_GET['template_id']);
		echo "<div class='updated'><p>" . t('Template deleted.') . "</p></div>";
...
if (count($templates)) {
	...
	foreach($templates as $template) {
		...
		<td><?php
			$link = "edit.php?page=article-templates/manage.php&amp;action=delete&amp;template_id=$template->ID";
