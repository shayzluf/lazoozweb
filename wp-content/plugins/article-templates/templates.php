...
<label for="template" style="font-size: 11px; font-weight: bold;"><?php _e("Templates: ", 'article-templates') ?></label>
<br /><br />
...
jQuery(function(){
	jQuery("#template").change(insertTemplate);
	...
	if(default_template) {
		jQuery("#template").val(default_template);
...
function insertTemplate(insertion_type) {
	...
	if(window.tinyMCE && jQuery("#content").css("display") == "none") {
		...
		if(getText(tinyMCE.get('content').getContent())) {
			...
			jQuery("#template").val("none");
			return;
		}
		tinyMCE.get('content').setContent(all_templates[template].replace(/\n/g,"<br />"));

	} else if(jQuery("#content")) {
		//If there is any content in the text area, don't insert the template
		if(getText(jQuery("#content").val())) {
			if(confirm("Cannot insert template - content already present. Click OK to delete existing content and add the template.")) jQuery("#content").val(all_templates[template]);
			return;
		}

		jQuery("#content").val(all_templates[template]);
	}
}
...
function templates_get_users_templates($user_id) {
    ...
    $query = <<<SQL
        ...
        WHERE post_type='template' AND post_author='$user_id'
...
function templates_get_default_template_for_user($user_id) {
    ...
    $user_selected = $wpdb->get_var("SELECT post_name FROM {$wpdb->posts} WHERE post_type='template' AND post_author='$user_id' AND menu_order='1' LIMIT 1");
